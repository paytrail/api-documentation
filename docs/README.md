...
The request body is exactly the same as in a normal [Payment-creation](#create) with the exception that only special characters: `_` and `-` can be used in `stamp` and that the `callbackUrls` is a required field.
...
| reference               | string                                      | <center>x</center> | Order reference. Maximum of 200 characters. If per-transaction settlement is used, the value must be a valid Finnish bank reference number.                                                                                                                                                                                                                                                              |
...
| providerDetails         | [ProviderDetails](#providerdetails)         | <center>-</center> | Pass properties to payment methods that support additional functionality. Content depends on payment method used.                                                                                                                                                                                                                                                                                        |
