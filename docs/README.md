...
| `refund pending` | Refund has been initated but is pending approval. Only used for merchants which require refund approvals. Never returned as a result, but can be received from the `GET /payments/{transactionId}`                                       |
...
| status        | string  | `new`, `ok`, `fail`, `pending`, `delayed` or `refund pending`. Statuses are desribed [above](#statuses).             |
